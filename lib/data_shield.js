...
class DataRegistry {
  ...
  scanContent(content) {
    ...
    for (var i = 0; i < this.patterns.length; i++) {
      for (var j = 0; j < this.patterns[i].patterns.length; j++) {
        try {
          var re = new RegExp(this.patterns[i].patterns[j], 'i');
          if (re.test(content)) {
  ...
  levelOf(classification) {
    var levels = { credential: 0, pii: 1, financial: 2, internal: 3, public: 4 };
...
class AccessController {
  ...
  check(sessionId, action, resource, content) {
    // 1. 路径分类
    var classification = this.registry.classify(resource);

    // 2. 内容扫描（如果有内容）
    var contentMatches = content ? this.registry.scanContent(content) : [];

    // 3. 计算风险等级
    var riskLevel = classification.level;
    if (contentMatches.length > 0) {
      var minLevel = Math.min.apply(null, contentMatches.map(function(m) { return { critical: 0, high: 1, medium: 2 }[m.severity] || 3; }));
  ...
  requestApproval(sessionId, action, resource, riskLevel) {
    return new Promise(function(resolve, reject) {
      var approvalId = 'app-' + Date.now().toString(36) + '-' + Math.random().toString(36).slice(2, 6);
  ...
  handleApproval(sessionId, approved) {
    var pending = this.pendingApprovals.get(sessionId);
...
export class DataShield {
  ...
  evaluate(request) {
    this.stats.checks++;
    var result = { allowed: true, reason: 'passed', riskLevel: 4 };

    // 检测是否是文件/数据访问
    var resource = request.resource || request.args || '';
    var action = request.action || request.type || 'read';
    var content = request.content || null;

    if (resource) {
      var check = this.access.check(request.session_id, action, resource, content);
  ...
  approve(sessionId, approved) {
    var r = this.access.handleApproval(sessionId, approved);
...
if (process.argv[1] && process.argv[1].endsWith('data_shield.js')) {
  ...
  (async function() {
    for (var i = 0; i < tests.length; i++) {
      ...
      if (r.approvalRequired) console.log('  ⚠️ 需要授权弹窗');
    }
